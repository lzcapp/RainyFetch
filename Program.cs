    ...
    internal static class Program {
        ...
        private static void Main() {
            ...
            foreach (var cpu in cpus) {
                ...
                Result.Add(new[] { CapacityConvert(cpu["L2CacheSize"])[0], "white" });
                Result.Add(new[] { " " + CapacityConvert(cpu["L2CacheSize"], 1)[1], "red" });
                Result.Add(new[] { " · L3: ", "red" });
                Result.Add(new[] { CapacityConvert(cpu["L3CacheSize"])[0], "white" });
                Result.Add(new[] { " " + CapacityConvert(cpu["L3CacheSize"], 1)[1], "red" });
                Result.Add(new[] { "\n", string.Empty });
                count++;
            }
            ...
            foreach (var gpu in gpus) {
                ...
                if (!string.IsNullOrEmpty(gpu["AdapterRAM"].Trim())) {
                    Result.Add(new[] { " · ", "red" });
                    Result.Add(new[] { CapacityConvert(gpu["AdapterRAM"])[0], "white" });
                    Result.Add(new[] { " " + CapacityConvert(gpu["AdapterRAM"])[1], "red" });
                }
            ...
            foreach (var mem in mems) {
                ...
                Result.Add(new[] { " · ", "red" });
                var capacity = CapacityConvert(mem["Capacity"]);
            ...
            count = 1;
            tab = string.Empty;
            var dskOn = (from dsk in dsks let size = CapacityConvert(dsk["Size"]) where !string.IsNullOrEmpty(size[0]) select dsk).ToList();
            ...
            foreach (var dsk in dskOn) {
                ...
                var capacity = CapacityConvert(dsk["Size"]);
                Result.Add(new[] { capacity[0], "white" });
                Result.Add(new[] { " " + capacity[1], "red" });
                Result.Add(new[] { "\n", string.Empty });
                count++;
            ...
            foreach (var net in netOn) {
                ...
                var speed = SpeedCovert(speed_w32);
                if (!string.IsNullOrEmpty(speed[0])) {
                    ...
                    if (speed[0] != "0") {
                        ...
                        Result.Add(new[] { "No Connection", "red" });
                    }
                }
                Result.Add(new[] { "\n", string.Empty });
                count++;
        ...
        */
        ...
        private static List<string> CapacityConvert(string s, int i = 0) {
        ...
        private static List<string> SpeedCovert(string s) {
